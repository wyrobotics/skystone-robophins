...
public class AutonBlueGood extends LinearOpMode {
        ...
        while(opModeIsActive()){
         ...
         //   telemetry.addData("start time: ", startTime);
           // telemetry.addData("runtine: ", System.currentTimeMillis());
            ...
            extender.setPower(-1);
            moveStraight(.18, false, 0.4); //old val .3, then .15
            extender.setPower(0);
            ...
            moveStraight(4 + (strafeDistance*.33) - (strafeDistance == 0 ? 0.2 : 0), true, 0.6);
    ...
    public void turnFast(double degrees, boolean counterclockwise){ //tileval is encoder val for one tile
        int dir = 1;
        if(!counterclockwise){
            dir = -1; //ccw = all negative
        }
